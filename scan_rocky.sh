#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════════
# SysKit Scanner - Rocky Linux Edition (Non-Root, Safe Mode)
# ═══════════════════════════════════════════════════════════════════════════════
# - Runs WITHOUT root privileges. Refuses to execute as root.
# - All commands are READ-ONLY; nothing is modified.
# - Uses modern tools (ip, ss, systemctl, dnf). Avoids deprecated ifconfig/netstat.
...
banner() {
    cat <<EOF
================================================================================
  SysKit Scanner v${VERSION}  |  Rocky Linux (Non-Root Safe Mode)
...
echo ""
echo "============================================"
echo "  SysKit Scanner - Rocky Linux Safe Mode"
echo "============================================"
echo ""
...
# ─── Begin Report ────────────────────────────────────────────────────────────
{
    ...
    run_section "SELinux Status" "getenforce 2>/dev/null || echo 'SELinux status command not available'"
    ...
    # 8. Package & Update Status (dnf instead of apt)
    echo ""
    echo "  >> [8/13] Package Updates"
    run_section "Available Updates (dnf)" "dnf check-update 2>/dev/null | head -n 40 || echo 'dnf check-update returned no updates or is unavailable'"

    # 9. Firewall Status (firewalld instead of ufw)
    echo ""
    echo "  >> [9/13] Firewall"
    run_section "Firewalld Status" "firewall-cmd --state 2>/dev/null || echo 'firewalld not running or not available'"
    run_section "Firewalld Active Zones" "firewall-cmd --get-active-zones 2>/dev/null || echo 'Cannot retrieve zones - skipped'"
    run_section "Firewalld Zone Rules" "firewall-cmd --list-all 2>/dev/null || echo 'Cannot list rules (may require privileges) - skipped'"
    ...
    run_section "Secure Log (last 50)" "tail -n 50 /var/log/secure 2>/dev/null || echo '/var/log/secure not readable - skipped'"
    run_section "Messages Log (last 50)" "tail -n 50 /var/log/messages 2>/dev/null || echo '/var/log/messages not readable - skipped'"
