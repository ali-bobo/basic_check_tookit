#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════════
# SysKit Scanner v2.0  —  Rocky Linux Edition (Non-Root, Safe Mode)
# ═══════════════════════════════════════════════════════════════════════════════
...
OS_INFO=$(grep "^PRETTY_NAME=" /etc/os-release 2>/dev/null | cut -d'"' -f2 || echo "Rocky Linux")
...
PRIVATE_IP="N/A"; PUBLIC_IP="N/A"; SELINUX_STATUS="N/A"
...
echo ""
echo "============================================"
echo "  SysKit Scanner v${VERSION} - Rocky Linux"
echo "============================================"
...
run_section "OS Release" "cat /etc/os-release 2>/dev/null || cat /etc/redhat-release 2>/dev/null"
...
run_section "SELinux Status" "getenforce 2>/dev/null || echo 'SELinux 指令不可用'"
...
run_section "Available Updates (dnf)" "dnf check-update 2>/dev/null | head -n 40 || echo 'dnf check-update 未回傳結果或不可用'"

echo "[9/13] 防火牆..." >&2
echo "" >> "$DETAIL_TMP"; echo "  >> [9/13] 防火牆 (Firewall)" >> "$DETAIL_TMP"
run_section "Firewalld Status" "firewall-cmd --state 2>/dev/null || echo 'firewalld 未執行或不可用'"
run_section "Firewalld Active Zones" "firewall-cmd --get-active-zones 2>/dev/null || echo '無法取得區域 — 可能需管理員權限'"
skip_section "Firewalld Zone Rules (完整規則)" "firewall-cmd --list-all 需要管理員權限" "sudo firewall-cmd --list-all"
...
run_section "Secure Log (last 50)" "tail -n 50 /var/log/secure 2>/dev/null || echo '/var/log/secure 不可讀（需管理員）— 已跳過'"
run_section "Messages Log (last 50)" "tail -n 50 /var/log/messages 2>/dev/null || echo '/var/log/messages 不可讀（需管理員）— 已跳過'"
...
case "$SELINUX_STATUS" in
    Enforcing)  add_cis "SELinux 狀態" "PASS" "Enforcing" "" ;;
    Permissive) add_cis "SELinux 狀態" "FAIL" "Permissive" "管理員: sudo setenforce 1" ;;
    Disabled)   add_cis "SELinux 狀態" "FAIL" "Disabled" "管理員: 啟用 SELinux" ;;
    *)          add_cis "SELinux 狀態" "SKIPPED" "無法取得" "確認 SELinux 已安裝" ;;
esac
